...
if not exist ".env" (
    ...
    for /f "delims=" %%i in ('"%VENV_PYTHON%" -c "from cryptography.fernet import Fernet; print(Fernet.generate_key().decode())"') do set ENCRYPTION_KEY=%%i
    ...
    for /f "delims=" %%i in ('"%VENV_PYTHON%" -c "import secrets; print(secrets.token_urlsafe(32))"') do set SECRET_KEY=%%i
...
echo 2. Run the app: "%VENV_PYTHON%" run.py
